...
class DataGridViewPrinter
{
    ...
    public DataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument, string aTitleText)
    {
        ...
        TheTitleFont = new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point);
        TheTitleColor = Color.Black;
        IsWithPaging = true;
        ...
        if (!ThePrintDocument.DefaultPageSettings.Landscape)//横向打印还是纵向打印
        {
    ...
    private void Calculate(Graphics g)//计算表格中单元格的宽和高
    {
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        {
            PageNumber++;
            string PageString = "第" + PageNumber.ToString() + "页 " + string.Format("打印时间:{0}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
    ...
    private bool DrawRows(Graphics g)  //画行
    {
        // Setting the LinePen that will be used to draw lines and rectangles (derived from the GridColor property of the DataGridView control)
        Pen TheLinePen = new Pen(TheDataGridView.GridColor, 1);
    ...
    public bool DrawDataGridView(Graphics g)//调用其它函数
    {
        try
        {
            Calculate(g);
